...
First, ensure the `tlc` directory exists adjacent to the input specification file and contains an `__init__.py` with the standard TLC header:
...
Re-render the context-free spec into an intermediate file at `tlc/module-name.no-context.md`, where `tlc` is a directory adjacent to the input specification file. For example, if processing `path/to/module-name.md`, save to `path/to/tlc/module-name.no-context.md`.
...
Save the response to `tlc/${module_name}.questions.md` in the `tlc` directory adjacent to the input specification file - overwrite if it exists.
...
Use the standard logging library. Logging in the CLI is INFO by default, but we store in `tlc/logs/${module_name}.log` (in the `tlc` directory adjacent to the input specification file). The format of the logs in the console will be to just show the message and nothing else. The format of the logs in the file will include the timestamp, logger name, log level and message. Make sure to create the `tlc/logs` directory if it doesn't exist.
