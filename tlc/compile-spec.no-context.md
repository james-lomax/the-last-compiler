...
## Interface from sanity-check-spec.md
...
When the spec is ready, `compile-spec` performs code generation one-shot - a single code generation prompt is used to produce the output file, which is saved to the code target file in a `tlc` directory adjacent to the input specification file.
